...
class RetrievalQuery(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_project_scoping(self) -> Self:
        # A query that asks *exclusively* for PROJECT memories without
        # saying which project fails closed here — there is no safe
        # fallback (returning every project's facts would be exactly the
        # cross-project leak this model exists to prevent). A *mixed*
        # type list is handled leniently by
        # sam.memory.engine.MemoryEngine._effective_types instead, since
        # dropping PROJECT there still leaves a useful result.
        if self.memory_types == (MemoryType.PROJECT,) and self.project_id is None:
            raise ValueError("retrieving only PROJECT memories requires project_id")
        return self
